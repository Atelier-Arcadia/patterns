...
- Update `SubmissionInput` in `SqliteGrimoire.addSubmission()` to persist `source`
- Update `mapSubmissionRow` to include `source`

## 2. Interface Extension
- Add submission methods to `Grimoire` interface (or create a new `SubmissionStore` interface) so `createServer()` can access them without depending on the concrete `SqliteGrimoire` class
- Update `createServer()` to accept the extended interface

## 3. New Tool Handler
- Create `createSuggestHandler` in `tools.ts` following the existing spell
- Two modes via `type` field: `"new"` (requires domainSlug, categorySlug) and `"modify"` (requires targetSpellId)
